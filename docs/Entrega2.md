...
- [ ] Búsqueda de mensajes dentro de grupos
- [ ] Archivos adjuntos
- [ ] Mensajes destacados/pinned

### Mejoras técnicas:
- [ ] Paginación de mensajes (cargar bajo demanda)
- [ ] Soft delete de usuarios (mantener mensajes históricos)
- [ ] Rate limiting en mutations
- [ ] Audit log de cambios en grupos
...
- Types generados: `convex/_generated/*`
- Configuración: `convex.json`

---

##  FAQ

**Q: ¿Por qué `authorClerkId` es optional en messages?**  
A: Los mensajes de seed usan usuarios ficticios que no existen en Clerk.

**Q: ¿Qué pasa si elimino un usuario de Clerk?**  
A: El webhook llama a `deleteByClerkId` pero los mensajes quedan con `authorClerkId` huérfano. Esto es intencional para mantener historial.

**Q: ¿Puedo tener múltiples admins en un grupo?**  
A: Sí, no hay límite de admins. Solo hay restricción de mínimo 1 admin.

**Q: ¿Cómo funciona la reactivity de Convex?**  
A: Convex mantiene subscripciones WebSocket. Cuando cambias datos, todos los queries afectados se re-ejecutan automáticamente.

**Q: ¿Por qué se guarda `author` (string) además de `authorClerkId`?**  
A: Por performance. Evita joins al mostrar mensajes. El nombre se snapshot al enviar.
